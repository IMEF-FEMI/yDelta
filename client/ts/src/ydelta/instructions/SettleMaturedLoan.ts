...
export const settleMaturedLoanInstructionDiscriminator = YdeltaInstructionTag.SettleMaturedLoan;

export type SettleMaturedLoanInstructionAccounts = {
  payer: PublicKey;
  market: PublicKey;
...
export type SettleMaturedLoanInstructionArgs = {
  /** 0 (or absent) → repay full outstanding. */
  repayAtomsMax: BN | bigint | number;
};

export function createSettleMaturedLoanInstruction(
  accounts: SettleMaturedLoanInstructionAccounts,
  args: SettleMaturedLoanInstructionArgs,
  programId: PublicKey = YDELTA_PROGRAM_ID,
): TransactionInstruction {
  const repayMax = BigInt(args.repayAtomsMax.toString());
  const data = Buffer.alloc(9);
  data.writeUInt8(settleMaturedLoanInstructionDiscriminator, 0);
