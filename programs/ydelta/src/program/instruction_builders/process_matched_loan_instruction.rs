...
/// Build a `ProcessMatchedLoan` instruction for a full secondary transfer.
#[allow(clippy::too_many_arguments)]
pub fn process_secondary_matched_loan_instruction(
    market: &Pubkey,
    payer: &Pubkey,
    queue_sequence: u64,
    ...
    let (existing_loan, _) = loan_pda(market, referenced_loan_sequence);
    let mut data = YdeltaInstruction::ProcessMatchedLoan.to_vec();
    ProcessMatchedLoanParams {
        matched_loan_sequence: queue_sequence,
        matched_loan_index_hint,
    }
    .serialize(&mut data)
    .unwrap();
    let mut accounts = vec![
        ...
        AccountMeta::new_readonly(global_config_pda().0, false),
        AccountMeta::new(*market, false),
        AccountMeta::new(existing_loan, false),
        // Writable so the cranker's `do_vault_settle` can mutate the
        // bank's share counters via marginfi.withdraw + .deposit when
        // the new lender is a risk-profile.
        AccountMeta::new(*debt_bank, false),
        AccountMeta::new_readonly(*marginfi_program, false),
        AccountMeta::new_readonly(system_program::id(), false),
...
/// Build a `ProcessMatchedLoan` instruction for a split secondary transfer.
#[allow(clippy::too_many_arguments)]
pub fn process_secondary_split_matched_loan_instruction(
    market: &Pubkey,
    payer: &Pubkey,
    queue_sequence: u64,
    referenced_loan_sequence: u64,
    _next_market_sequence: u64,
    debt_bank: &Pubkey,
    collateral_bank: &Pubkey,
    debt_oracles: &[Pubkey],
    collateral_oracles: &[Pubkey],
    ...
    let (existing_loan, _) = loan_pda(market, referenced_loan_sequence);
    let (split_loan, _) = crate::state::loan::split_loan_pda(market, queue_sequence);
    let mut data = YdeltaInstruction::ProcessMatchedLoan.to_vec();
    ProcessMatchedLoanParams {
        matched_loan_sequence: queue_sequence,
        matched_loan_index_hint,
    ...
    accounts.push(AccountMeta::new(split_loan, false));
    Instruction {
        program_id: crate::id(),
        accounts,
        data,
