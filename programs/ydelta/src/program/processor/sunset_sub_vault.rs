//! `SunsetSubVault` — vault-admin sunset for a sub-vault. Signer is
//! the vault admin (via `SunsetSubVaultContext`). Sets `is_sunset = 1`,
...
//! `AdminCancelSubVaultOrder` and `RemoveSubVault` both gate on this
//! flag.
...
use crate::validation::loaders::SunsetSubVaultContext;
...
pub fn process_sunset_sub_vault(
    ...
    let params = SunsetSubVaultParams::try_from_slice(data)?;
    let SunsetSubVaultContext { _payer: _, vault } = SunsetSubVaultContext::load(accounts)?;
    ...
    require!(
        ...
        "sunset_sub_vault: sub_vault_id {} not found",
        params.sub_vault_id
    )?;
